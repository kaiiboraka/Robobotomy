...
public partial class LimbSelect : Node
{
	...
	private void ThrowLimb(Node3D currentNode)
	{
		Vector3 mousePosition = mainCamera.ProjectPosition(GetViewport().GetMousePosition(),
			Math.Abs(mainCamera.Position.Z - bodyObjects[LimbTypes.Torso].Position.Z));
		Vector3 direction = bodyObjects[LimbTypes.Torso].Position
														.DirectionTo(new Vector3(mousePosition.X, mousePosition.Y,
															 0));
